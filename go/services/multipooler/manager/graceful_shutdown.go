...
// would just hang again. The orchestrator's deadline-fired failover relies
// on this process actually exiting by its own deadline.
//
...
// past it eats into the orchestrator's deadline-fired-failover budget. OTel
// exporters run on independent goroutines, so the flush typically succeeds
// even when the main GracefulShutdown goroutine is hung.
...
// gracefulShutdownAnnouncedDeadline is the wall-clock budget announced to the
// orchestrator as shutdown_deadline. The orchestrator interprets it as a
// duration relative to receipt time and arms its deadline-fired failover
// timer against this value. The pooler must be done — process exited or at
// least postgres stopped — by the time this elapses.
//
...
// Compile-time assertion that the announced deadline meets the orchestrator's
// minimum. If gracefulShutdownAnnouncedDeadline ever drops below
// constants.MinShutdownDeadline, the subtraction goes negative and conversion
// to uint fails to compile.
...
// fit inside the announced shutdown_deadline.
//
...
//     deadline. This is what makes the announced deadline binding from the
//     orchestrator's perspective: if we hang past it, the orchestrator will
//     deadline-fire failover, and we must not still be alive serving writes
//     when that happens. Operators who want to exit faster than the deadline
//     should send SIGKILL.
//
...
//     the orchestrator-facing announcement so the deadline budget the
//     orchestrator sees does not include drain time.
//
//  3. Set lifecycleSignal=SHUTTING_DOWN with the deadline and broadcast so
//     the orchestrator receives the announcement and arms its timer.
//
...
//
//  6. Set lifecycleSignal=STOPPED and broadcast as a terminal snapshot.
//     The orchestrator's deadline timer is cancelled by this snapshot;
//     failover proceeds against a leader that has cleanly resigned.
func (pm *MultiPoolerManager) GracefulShutdown(ctx context.Context) {
	...
	// something else.
	//
	// However, we will not wait indefinitely since we have announced a deadline
	// to the orchestrator and must exit by that deadline to trigger failover.
	// This is a preventive measure to not risk causing a split-brain scenario
	// by still being alive and serving writes after the orchestrator has failed
	// over to a new primary.
	//
	// The safety-net fires safetyNetTelemetryFlushTimeout *before* the
	// announced deadline, so that flush + os.Exit complete by the deadline the
	// orchestrator is timing against. Without this offset the process could
	// linger past the announced deadline by the flush duration, which would be
	// a contract violation toward the orchestrator's deadline-fired failover.
	...
	// finish, force-close reserved on grace-period timeout. Done before the
	// SHUTTING_DOWN announce so the orchestrator's deadline measures only
	// the post-drain remainder.
	...
	// Phase 2: announce. The orchestrator arms its deadline timer on this
	// snapshot and bypasses the leader-down suppression heuristics. Not
	// terminal: subscribers continue to receive updates through the slow
	// phases below.
