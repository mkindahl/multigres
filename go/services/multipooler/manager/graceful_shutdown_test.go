...
// readLifecycleState returns a coherent snapshot of the lifecycle fields
// under pm.mu so the test never races the production setter.
func readLifecycleState(pm *MultiPoolerManager) (clustermetadatapb.LifecycleSignal, time.Time) {
	pm.mu.Lock()
	defer pm.mu.Unlock()
	return pm.lifecycleSignal, pm.lifecycleDeadlineAt
}
...
func TestGracefulShutdown_HappyPath(t *testing.T) {
	...
	sig, deadlineAt := readLifecycleState(pm)
	assert.Equal(t, clustermetadatapb.LifecycleSignal_LIFECYCLE_SIGNAL_STOPPED, sig,
		"final lifecycle state must be STOPPED")
	assert.True(t, deadlineAt.IsZero(), "STOPPED clears the deadline")
}
...
func TestGracefulShutdown_AllModesFail(t *testing.T) {
	...
	assert.True(t, deadlineAt.IsZero())
}
...
// TestGracefulShutdown_AnnouncesShuttingDownWithDeadline verifies that the
// announcement happens before the slow phases run. The test uses a pgctld stub
// that snapshots the manager's lifecycle state at the moment Stop is called;
// at that point the lifecycle must already be SHUTTING_DOWN with a non-zero
// deadline.
func TestGracefulShutdown_AnnouncesShuttingDownWithDeadline(t *testing.T) {
	var (
		observedSignal     clustermetadatapb.LifecycleSignal
		observedDeadlineAt time.Time
	...
	pgctld := &recordingPgctldClient{
		stopFn: func(_ string) error {
			observedOnce.Do(func() {
				observedSignal, observedDeadlineAt = readLifecycleState(pm)
			})
			return nil
		},
	}
	...
	assert.Equal(t, clustermetadatapb.LifecycleSignal_LIFECYCLE_SIGNAL_SHUTTING_DOWN, observedSignal,
		"announcement must precede pgctld.Stop")
	assert.True(t, observedDeadlineAt.After(beforeStart),
		"deadlineAt must be set in the future at announcement time")
}
